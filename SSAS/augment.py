...
def augment(data,
    ...
    # cov1 = Covariances().transform(x)
    # indx = ElectrodeSelection(nelec=3, metric='riemann', n_jobs=1).fit(cov1,y,sample_weight=None).subelec_
    # mask = np.ones((n_trials, n_channels, n_timepoints))
    # mask_c = prng.random(len(indx))
    # mask[:, indx, :] = ((1.0 + mask_c)).reshape(-1, 1).repeat(n_timepoints, axis=1)
    # x_aug = mask * x
    # Get the feature
    ...
    Ca_noTral,new_data_x = ca.fit_transform(x_aug)
    # tan = data.data[str(subject)]['tanTrans'].transform(Ca_noTral)

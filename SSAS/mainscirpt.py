...
class DFN(nn.Module):
  def __init__(self, input_size, hidden_size, use_bottleneck=True, bottleneck_dim=100, radius=10.0, class_num=1000,t:Optional[float] = 0.1):
    ...
    self.fc = feat_classifier(bottleneck_dim, class_num)
  ...
  def forward(self, x):
    ...
    #   x = self.radius * x / (torch.norm(x, dim=1, keepdim=True) + 1e-10)
      x = temperature_normalization(x, self.t)
